...
const INVOICE_SELECT = "id, seq_number, invoice_number, currency, total, due_date, nf_status, nf_amount_brl, jobs(name, nf_description, clients(name, legal_name, cnpj, address, nf_rules))"
...
type InvoiceRow = {
  ...
  jobs: {
    name: string; nf_description: string | null
    clients: { name: string; legal_name: string | null; cnpj: string | null; address: string | null; nf_rules: string | null } | null
  } | null
}
...
function buildFromInvoice(invoice: InvoiceRow, settings: UserSettings | null, origin: string) {
  ...
  const built = buildNfRequest({
    ...
    accountantName: settings?.accountant_name ?? null,
    provider: {
      legalName: settings?.legal_name ?? null,
      cnpj: settings?.cnpj_cpf ?? null,
      bankName: settings?.bank_name ?? null,
      bankAccount: settings?.bank_account_number ?? null,
      pixKey: settings?.pix_key ?? null,
    },
    invoicePdfUrl: `${process.env.NEXT_PUBLIC_SITE_URL ?? origin}/api/invoices/pdf?id=${invoice.id}&lang=pt`,
  })
...
export async function POST(request: NextRequest) {
  ...
  const built = buildFromInvoice(invoice, settings, request.nextUrl.origin)
...
export async function GET(request: NextRequest) {
  ...
  const { subject, body } = buildFromInvoice(invoice, settings, request.nextUrl.origin)
